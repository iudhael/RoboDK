...
def send_instruction(instruction):
    command = (MRCP_START_FRAME + MRCP_COMMAND_EXECUTE + instruction + MRCP_END_FRAME)
    log('sending instruction: ' + command)
    ser.write(command.encode())
    # ser.flush()
"""def send_instruction(instruction):
    command = (MRCP_START_FRAME + MRCP_COMMAND_EXECUTE + instruction + MRCP_END_FRAME).encode('ascii')
    log('sending instruction: ' + command.decode('ascii'))  # Log en string
    ser.write(command)  # Envoi en bytes"""
...
while 1:
    ...
    if select.select([sys.stdin, ], [], [], 0.0)[0]:
        ...
        """
        if msvcrt.kbhit():
            #data = msvcrt.getche().decode()  # Lire un caractère à la fois
            data = input()"""
        log('R: ' + data)
        ...
        if connected is False:
            ...
            if commands[0] == 'CONNECT':
                ...
                connected = True
                ser = serial.Serial(commands[1], baudrate=9600, timeout=5)
                ser.setRTS(False)
            ...
            for i in range(0, 6):
                instruction += 'R' + str(i) + ' ' + format_number(commands[i + 1]) + ' '
